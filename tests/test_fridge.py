...
# This is just the simplest example. Normally if init method is so simple we
# would not test it.
def test_instantiation(loaded_fridge):
    ...
    assert loaded_fridge.inverter == "INV4403PH"
...
# ((inverter, load, temp, base_speed), fan_speed_setting)
...
# Test outgoing command was sent out to the inverter and a correct response
#  was received.
@pytest.mark.parametrize("mock_inverter, expected", test_data,
                         indirect=["mock_inverter"])
def test_set_target_speed(loaded_fridge, mock_inverter, expected):
    """Test behavior of fridge when current speed matches set speed."""

    loaded_fridge.inverter = mock_inverter
    loaded_fridge.set_target_speed(expected)

    mock_inverter.set_target_speed.assert_called_once_with(expected)


# Test outgoing command raises exception on incorrect response from inverter.
@pytest.mark.parametrize("mock_inverter", [1000, 1200, 1300, 1400],
                         indirect=["mock_inverter"])
def test_raises_exception_if_curr_spd_not_target(loaded_fridge, mock_inverter):
    ...
    with pytest.raises(ValueError,
        ...
        loaded_fridge.inverter = mock_inverter
        loaded_fridge.set_target_speed(1500)
...
class TestApiCalls(object):
    ...
    def test_order_beer_response_200(self, loaded_fridge, mock_response_200, monkeypatch):
        ...
        assert loaded_fridge.order_beer(self.num_of_bottles).status_code == 200
